...
const StatisicsStorage = require('./StatisticsStorage');
...
function fileReceiver(req, res, _) {
...
app.get('/', viewClones);
app.get('/stats', viewStatistics);

const server = app.listen(PORT, () => { console.log('Listening for files on port', PORT); });

// Page generation for viewing statistics
// --------------------

function getAverageStats() {
  let statisticsStore = StatisicsStorage.getInstance();

  return statisticsStore.getOutput()
}
...
function getGraph() {
  ...
  let output = '';
  output += '<canvas id="myChart" style="width:100%;max-width:700px;heigth: 500px;"></canvas>\n';
  output += '<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>\n';
  output += '<script>\n';
  output += 'new Chart(\'myChart\', { type: "line", data: {' + statisticsStore.getGraphData() + '}, options: {} });\n';
  output += '</script>\n';

  return output;
}

function viewStatistics(_, res, _) {
  let page = '<HTML><HEAD><TITLE>CodeStream Clone Detector</TITLE></HEAD>\n';
  page += '<BODY><H1>CodeStream Statistics</H1>\n';
  page += '<P>' + getAverageStats() + '</P>\n';
  page += getGraph() + '\n';
  page += '</BODY></HTML>';
  res.send(page);
...
function maybeNoteAverage(file, cloneDetector, statisticsStore) {
  if (0 == cloneDetector.numberOfProcessedFiles % 100) {
    statisticsStore.noteAverage();
  }
...
function processFile(filename, contents) {
  ...
  return Promise.resolve({ name: filename, contents: contents })
    ...
    .then((file) => statisticsStore.addTime(file, Timer))
    .then(PASS((file) => lastFile = file))
    .then(PASS((file) => maybePrintStatistics(file, cd, cloneStore)))
    .then(PASS((file) => maybeNoteAverage(file, cd, statisticsStore)))
