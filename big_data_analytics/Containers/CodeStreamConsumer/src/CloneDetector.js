...
class CloneDetector {
  ...
  #filterCloneCandidates(file, compareFile) {
    ...
    let instances = file.chunks.map((chunk) => {
      return compareFile.chunks.filter((compareChunk) => {
        return this.#chunkMatch(chunk, compareChunk);
      }, this).map(compareChunk => {
  ...
  #expandCloneCandidates(file) {
    ...
    let instances = file.instances.reduce((accumulator, srcClone) => {
      let merged = false;

      accumulator.every(clone => {
        merged = clone.maybeExpandWith(srcClone);
        return !merged;
      });

      if (!merged)
        accumulator.push(srcClone);
  ...
  #consolidateClones(file) {
    ...
    file.instances = file.instances.reduce((accumulator, srcClone) => {
      if (typeof accumulator.find((clone) => srcClone.equals(clone)) === 'undefined') {
        accumulator.push(srcClone);
      }
